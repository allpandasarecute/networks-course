...
#### Решение
Последний отправится через $\dfrac{L}{R}(P-1)$ после первого, идти будет $N \dfrac{L}{R}$ ==> обобщение $\dfrac{L}{R}(P + N - 1)$
...
#### Решение
Скорость в таком случае будет определяться самым медленным каналом ==> время $\dfrac{5 \cdot 8 \cdot 10^6}{2 \cdot 10^5} = 200$ секунд
...
#### Решение
$\sum_{i=12}^{20} \binom{60}{i} \cdot 0.2^i \cdot 0.8^{60-i} \approx 0.54655$
...
2. Если $x = \dfrac{L}{R}$. Тогда общая задержка равна $\dfrac{L}{R(1-I)} = \dfrac{x}{1-I} = \dfrac{x}{1-\alpha x}$
